...
// FormatNewUserConfig generates a pretty string representation of a kafka-go
// user config.
func FormatNewUserConfig(config kafka.UserScramCredentialsUpsertion) string {
	content, err := json.MarshalIndent(config, "", "  ")
	if err != nil {
		log.Warnf("Error marshalling user config: %+v", err)
		return "Error"
	}
